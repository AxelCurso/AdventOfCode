...
int main(int argc, char const *argv[]) {
    ...
    for (int i = 0; i < instructions.size(); i++){
        if (instructions[i].first == 'x') {
            std::cout << "folding on vertical x: " << instructions[i].second << std::endl;
            paper = foldVertical(paper, instructions[i].second);
        }
        else {
            std::cout << "folding on horizontal y: " << instructions[i].second << std::endl;
            paper = foldHorizontal(paper, instructions[i].second);
        }
        printPaper(paper);
    }
