...
std::vector<std::vector<int> > diagAddedToGrid(std::vector<std::vector<int> > grid, t_ventsLine vl){
    std::vector<std::vector<int> > ret;

    ret = grid;
    if (vl.start.x < vl.finish.x){
        if (vl.start.y < vl.finish.y){
            for (int i = vl.start.x; i <= vl.finish.x; i++){
                ret[vl.start.y+i-vl.start.x][i]++;
            }
        } else {
            for (int i = vl.start.x; i <= vl.finish.x; i++){
                ret[vl.start.y-i+vl.start.x][i]++;
        ...
        if (vl.start.y < vl.finish.y){
            for (int i = vl.finish.x; i <= vl.start.x; i++){
                ret[vl.finish.y-i+vl.finish.x][i]++;
            }
        } else {
            for (int i = vl.finish.x; i <= vl.start.x; i++){
                ret[vl.finish.y+i-vl.finish.x][i]++;
...
int main(int argc, char const *argv[]) {
    ...
    std::vector<t_ventsLine>        vOnlyDiags;
    t_point                         maxXnY;
    ...
    for (int i = 0; i < vVL.size(); i++){
        if (isStraight(vVL[i])){
            ...
            vOnlyDiags.push_back(vVL[i]);
        }
    }
    maxXnY = getMaxXnY(vVL);
    ...
    for (int i = 0; i < vOnlyDiags.size(); i++){
        grid = diagAddedToGrid(grid, vOnlyDiags[i]);
    }
    // printGrid(grid);
